...
export default function QuoteForm() {
  ...
  async function submit(e: FormEvent<HTMLFormElement>) {
    ...
    const payload = { ...Object.fromEntries(new FormData(form).entries()), lineItems: items };
    try {
      const res = await fetch('/api/contact', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(payload) });
      const data = await res.json(); if (!res.ok) throw new Error(data.message || 'Unable to send quote request.');
      setStatus('success'); setNotice('Your quote request has been sent to sales@koken-inc.cn and info@koken-inc.cn.'); clear(); form.reset();
